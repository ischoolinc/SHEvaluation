...
namespace SmartSchool.Evaluation.Reports
{
    class NotReachStandardList
    {
        ...
        {
            MotherForm.SetStatusBarMessage("未達補考標準名單產生完成");
            Completed("未達補考標準名單", (Workbook)e.Result);
        }

        void _BGWResitList_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            MotherForm.SetStatusBarMessage("未達補考標準名單產生中", e.ProgressPercentage);
