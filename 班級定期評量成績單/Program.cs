    ...
    public class Program
    {
        ...
        private static void Program_Click(object sender_, EventArgs e_)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                System.Diagnostics.Trace.WriteLine(DateTime.Now.ToString("HH:mm:ss") + " 班級定期評量成績單(測試版)產生 S");
                bkw.ProgressChanged += delegate (object sender, System.ComponentModel.ProgressChangedEventArgs e)
                {
                    FISCA.Presentation.MotherForm.SetStatusBarMessage(" 班級定期評量成績單(測試版)產生中", e.ProgressPercentage);
                    System.Diagnostics.Trace.WriteLine(DateTime.Now.ToString("HH:mm:ss") + " 班級定期評量成績單(測試版)產生 " + e.ProgressPercentage);
                };
                bkw.RunWorkerCompleted += delegate
                {
                    System.Diagnostics.Trace.WriteLine(DateTime.Now.ToString("HH:mm:ss") + " 班級定期評量成績單(測試版)產生 E");
                    ...
                    if (exc != null)
                    {
                        throw new Exception("產生班級定期評量成績單(測試版)發生錯誤", exc);
                    }
                        #region 儲存檔案
                    ...
                    catch
                    {
                        ...
                        #endregion
                        FISCA.Presentation.MotherForm.SetStatusBarMessage("班級定期評量成績單(測試版)產生完成。", 100);
                    if (overflowRecords.Count > 0)
                        MessageBox.Show(err);
                };
        ...
        internal static void CreateFieldTemplate()
        {
            ...
            #region 儲存檔案
            string inputReportName = "班級定期評量成績單(測試版)合併欄位總表";
            string reportName = inputReportName;
