...
namespace ClassSemesterScoreReportFixed_SH
{
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            var btn = K12.Presentation.NLDPanels.Class.RibbonBarItems["資料統計"]["報表"]["成績相關報表"]["班級學期成績單(測試版)"];            
            ...
            cat1.Add(new RibbonFeature(regCode,"班級學期成績單(測試版)"));
        ...
        private static void Program_Click(object sender_, EventArgs e_)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                bkw.RunWorkerCompleted += delegate
                {
                    ...
                    #region 儲存檔案
                    string inputReportName = "班級學期成績單";
                ...
                bkw.DoWork += delegate(object sender, System.ComponentModel.DoWorkEventArgs e)
                {
                   ...
                   // 取得學期科目成績
                    ...
                    try
                    {
                        ...
                        //        if(studentSubjectScoreDict.ContainsKey(studentID))                                
                        //        {
                        ...
                        //                        printSubjectSum += subjScore.Score; ;//計算總分
                        //                            printSubjectCount++;
                        //                            //計算加權總分
                        //                            printSubjectSumW += subjScore.Score * decimal.ToInt16((decimal)subjScore.Credit);
                        //                            printSubjectCreditSum += decimal.ToInt16((decimal)subjScore.Credit);
                        ...
                        //                                {
                        //                                    //各科目班排名
                        //                                    key = "班排名" + studentRec.RefClass.ClassID + "^^^" + subjScore.Subject + "^^^" + subjScore.Level ;
                        ...
                        //                                    ranks[key].Add(subjScore.Score);
                        //                                    rankStudents[key].Add(studentID);
                        //                                }
                        //                                if (studentRec.Department != "")
                        //                                {
                        //                                    //各科目科排名
                        //                                    key = "科排名" + studentRec.Department + "^^^" + gradeyear + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                        //                                    if (!ranks.ContainsKey(key)) ranks.Add(key, new List<decimal>());
                        //                                    if (!rankStudents.ContainsKey(key)) rankStudents.Add(key, new List<string>());
                        //                                    ranks[key].Add(subjScore.Score);
                        //                                    rankStudents[key].Add(studentID);
                        //                                }
                        //                                //各科目全校排名
                        //                                key = "全校排名" + gradeyear + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                        //                                if (!ranks.ContainsKey(key)) ranks.Add(key, new List<decimal>());
                        //                                if (!rankStudents.ContainsKey(key)) rankStudents.Add(key, new List<string>());
                        //                                ranks[key].Add(subjScore.Score);
                        //                                rankStudents[key].Add(studentID);
                        ...
                        //                            tag1SubjectSum += subjScore.Score;//計算總分
                        //                            tag1SubjectCount++;
                        //                            //計算加權總分
                        //                            tag1SubjectSumW += subjScore.Score * decimal.ToInt16((decimal)subjScore.Credit);
                        //                            tag1SubjectCreditSum += decimal.ToInt16((decimal)subjScore.Credit);
                        //                            //各科目類別1排名
                        //                            if (rank && studentRec.Status == "一般")//不在過濾名單且為一般生才做排名
                        //                            {
                        //                                if (conf.PrintSubjectList.Contains(subjectName))//是列印科目才算科目排名                                                
                        //                                {
                        //                                    key = "類別1排名" + tag1ID + "^^^" + gradeyear + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                        //                                    if (!ranks.ContainsKey(key)) ranks.Add(key, new List<decimal>());
                        //                                    if (!rankStudents.ContainsKey(key)) rankStudents.Add(key, new List<string>());
                        //                                    ranks[key].Add(subjScore.Score);
                        //                                    rankStudents[key].Add(studentID);
                        ...
                        //                            tag2SubjectSum += subjScore.Score;//計算總分
                        //                            tag2SubjectCount++;
                        //                            //計算加權總分
                        //                            tag2SubjectSumW += subjScore.Score * decimal.ToInt16((decimal)subjScore.Credit);
                        //                            tag2SubjectCreditSum += decimal.ToInt16((decimal)subjScore.Credit);
                        //                            //各科目類別2排名
                        //                            if (rank && studentRec.Status == "一般")//不在過濾名單且為一般生才做排名
                        //                            {
                        //                                if (conf.PrintSubjectList.Contains(subjectName))//是列印科目才算科目排名                                                
                        //                                {
                        //                                    key = "類別2排名" + tag2ID + "^^^" + gradeyear + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                        //                                    if (!ranks.ContainsKey(key)) ranks.Add(key, new List<decimal>());
                        //                                    if (!rankStudents.ContainsKey(key)) rankStudents.Add(key, new List<string>());
                        //                                    ranks[key].Add(subjScore.Score);
                        //                                    rankStudents[key].Add(studentID);
                        ...
                        foreach (ClassRecord classRec in selectedClasses)
                        {
                            ...
                            foreach (StudentRecord stuRec in classRec.Students)
                            {
                                ...
                                // 處理類別
                                if (Global._TempGroup1RankDict.ContainsKey(studentID))
                                {
                                    if (Global._TempGroup1RankDict[studentID].ContainsKey("學業_類別1"))
                                    {
                                        if (!tagName1List.Contains(Global._TempGroup1RankDict[studentID]["學業_類別1"].Name))
                                            tagName1List.Add(Global._TempGroup1RankDict[studentID]["學業_類別1"].Name);
                                    }

                                    if (Global._TempGroup1RankDict[studentID].ContainsKey("學業_類別2"))
                                    {
                                        if (!tagName2List.Contains(Global._TempGroup1RankDict[studentID]["學業_類別2"].Name))
                                            tagName2List.Add(Global._TempGroup1RankDict[studentID]["學業_類別2"].Name);
                                    }
                                }
                            }
                            row["類別排名1"] = string.Join("/", tagName1List.ToArray());
                            row["類別排名2"] = string.Join("/", tagName2List.ToArray());
                                        ...
                                        if (conf.PrintSubjectList.Contains(subjectName))
                                        {
                                            ...
                                            foreach (StudentRecord stuRec in classRec.Students)
                                            {
                                                ...
                                                if (ClassStuNum > conf.StudentLimit)
                                                    break;
                                                string studentID = stuRec.StudentID;
                                                if (StudSubjRankData.ContainsKey(studentID))
                                                {
                                                    string key1 = subjectName + subjectNumber;
                                                    
                                                    //科目班排
                                                    if (Global._TempSubjClassRankDict.ContainsKey(studentID))
                                                        if (Global._TempSubjClassRankDict[studentID].ContainsKey(key1))
                                                        {
                                                            row["班排名" + ClassStuNum + "-" + subjectIndex] = Global._TempSubjClassRankDict[studentID][key1].Rank;

                                                            // 班排母數
                                                            row["班排名母數" + ClassStuNum + "-" + subjectIndex] = Global._TempSubjClassRankDict[studentID][key1].RankT;
                                                            row["科目成績" + ClassStuNum + "-" + subjectIndex] = Global._TempSubjClassRankDict[studentID][key1].Score;
                                                        }
                                                }
                                                ...
                                                //if (studentSubjectScoreDict.ContainsKey(studentID))
                                                //{
                                                //    if (studentSubjectScoreDict[studentID].ContainsKey(subjectName))
                                                //    {
                                                        // 
                                                        ...
                                                        //    //row["科目成績" + ClassStuNum + "-" + subjectIndex] = subjScore.Score;
                                                        //    //#region 班排名及落點分析
                                                        //    //if (stuRec.RefClass != null)
                                                        //    //{
                                                        //    //    key = "班排名" + stuRec.RefClass.ClassID + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                                                        //    //    if (rankStudents.ContainsKey(key) && rankStudents[key].Contains(studentID))//明確判斷學生是否參與排名
                                                        //    //    {
                                                        //    //        row["班排名" + ClassStuNum + "-" + subjectIndex] = ranks[key].IndexOf(subjScore.Score) + 1;
                                                        ...
                                                        //    //{
                                                        //    //    key = "科排名" + stuRec.Department + "^^^" + gradeYear + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                                                        //    //    if (rankStudents.ContainsKey(key) && rankStudents[key].Contains(studentID))//明確判斷學生是否參與排名
                                                        //    //    {
                                                        //    //        row["科排名" + ClassStuNum + "-" + subjectIndex] = ranks[key].IndexOf(subjScore.Score) + 1;
                                                        ...
                                                        //    //if (rankStudents.ContainsKey(key) && rankStudents[key].Contains(studentID))//明確判斷學生是否參與排名
                                                        //    //{
                                                        //    //    row["全校排名" + ClassStuNum + "-" + subjectIndex] = ranks[key].IndexOf(subjScore.Score) + 1;
                                                        ...
                                                        //    //{
                                                        //    //    key = "類別1排名" + studentTag1Group[studentID] + "^^^" + gradeYear + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                                                        //    //    if (rankStudents.ContainsKey(key) && rankStudents[key].Contains(studentID))//明確判斷學生是否參與排名
                                                        //    //    {
                                                        //    //        row["類別1排名" + ClassStuNum + "-" + subjectIndex] = ranks[key].IndexOf(subjScore.Score) + 1;
                                                        ...
                                                        //    //{
                                                        //    //    key = "類別2排名" + studentTag2Group[studentID] + "^^^" + gradeYear + "^^^" + subjScore.Subject + "^^^" + subjScore.Level;
                                                        //    //    if (rankStudents.ContainsKey(key) && rankStudents[key].Contains(studentID))//明確判斷學生是否參與排名
                                                        //    //    {
                                                        //    //        row["類別2排名" + ClassStuNum + "-" + subjectIndex] = ranks[key].IndexOf(subjScore.Score) + 1;
...
foreach (StudentRecord stuRec in classRec.Students)
{
    ...
    if (Global._TempClassRankDict.ContainsKey(studentID))
    {
        if (Global._TempClassRankDict[studentID].ContainsKey("學業"))
        {
            row["加權平均" + ClassStuNum] = Global._TempClassRankDict[studentID]["學業"].Score;
            row["加權平均班排名" + ClassStuNum] = Global._TempClassRankDict[studentID]["學業"].Rank;
            row["加權平均班排名母數" + ClassStuNum] = Global._TempClassRankDict[studentID]["學業"].RankT;
        }
    }
    
    if(Global._TempDeptRankDict.ContainsKey(studentID))
    {
        if(Global._TempDeptRankDict[studentID].ContainsKey("學業"))
        {
            row["加權平均科排名" + ClassStuNum] = Global._TempDeptRankDict[studentID]["學業"].Rank;
            row["加權平均科排名母數" + ClassStuNum] = Global._TempDeptRankDict[studentID]["學業"].RankT;
        }
    }

    if (Global._TempGradeYearRankDict.ContainsKey(studentID))
    {
        if (Global._TempGradeYearRankDict[studentID].ContainsKey("學業"))
        {
            row["加權平均全校排名" + ClassStuNum] = Global._TempGradeYearRankDict[studentID]["學業"].Rank;
            row["加權平均全校排名母數" + ClassStuNum] = Global._TempGradeYearRankDict[studentID]["學業"].RankT;
        }
    }
    ...
    {
        string cKey1 = "學業_類別1";
        if (Global._TempGroup1RankDict[studentID].ContainsKey(cKey1))
        {
            row["類別1加權平均" + ClassStuNum] = Global._TempGroup1RankDict[studentID][cKey1].Score;
            row["類別1加權平均排名" + ClassStuNum] = Global._TempGroup1RankDict[studentID][cKey1].Rank;
            row["類別1加權平均排名母數" + ClassStuNum] = Global._TempGroup1RankDict[studentID][cKey1].RankT;        
        }

        string cKey2 = "學業_類別2";
        if (Global._TempGroup1RankDict[studentID].ContainsKey(cKey2))
        {
            row["類別2加權平均" + ClassStuNum] = Global._TempGroup1RankDict[studentID][cKey2].Score;
            row["類別2加權平均排名" + ClassStuNum] = Global._TempGroup1RankDict[studentID][cKey2].Rank;
            row["類別2加權平均排名母數" + ClassStuNum] = Global._TempGroup1RankDict[studentID][cKey2].RankT;
        }
    }
