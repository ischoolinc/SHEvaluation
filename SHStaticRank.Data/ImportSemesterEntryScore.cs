            ...
            foreach (string field in e.SelectFields)
            {
                ...
                switch (field)
                {
                    ...
                    case "類組排名母數":
                        ...
                        {
                            e.WarningFields.Add(field, "無法單獨匯入排名母數而不匯入名次。");
                        }
                        else
                            if (value == "" || !int.TryParse(value, out t))
                            {
                    ...
                    case "校排名":
                        ...
                        else
                        {
                            ...
                            {
                                e.WarningFields.Add(field, "匯入排名時應同時匯入母數，如果沒有同時匯入母數，母數欄位將會被清空。");
                            }
                        }
                        break;
                    case "類組排名":
            ...
            foreach (string id in id_Rows.Keys)
            {
                ...
                foreach (RowData row in id_Rows[id])
                {
                    ...
                    foreach (string key in row.Keys)
                    {
                        ...
                        if (e.ImportFields.Contains("類組排名"))
                        {
                            hastag1Rating = true;
                            if (row["類組排名"] == "")
                            {
                            ...
                            else
                            {
                                var item = doc.CreateElement("Item");
                                item.SetAttribute("成績", "" + (e.ImportFields.Contains("學業") ? row["學業"] : ((semesterScoreDictionary.ContainsKey(sy) && semesterScoreDictionary[sy].ContainsKey(se) && semesterScoreDictionary[sy][se].ContainsKey("學業")) ? semesterScoreDictionary[sy][se]["學業"].Score.ToString() : "0")));
                                if (e.ImportFields.Contains("類組排名母數"))
                                {
                                    item.SetAttribute("成績人數", "" + row["類組排名母數"]);
                                    tag1Rating.SetAttribute("範圍人數", "" + row["類組排名母數"]);
                                }
                                else
                                {
                                    item.SetAttribute("成績人數", "0");
                                ...
                                item.SetAttribute("排名", "" + row["類組排名"]);
                                item.SetAttribute("分項", "學業");
                                tag1Rating.AppendChild(item);
                            }
                        }
                        if (hasclassRating || hasdeptRating || hasschoolRating || hastag1Rating)
                        {
