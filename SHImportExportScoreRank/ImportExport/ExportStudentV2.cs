    ...
    public partial class ExportStudentV2 : FISCA.Presentation.Controls.BaseForm, SmartSchool.API.PlugIn.Export.ExportWizard
    {
        ...
        public ExportStudentV2(string title, Image img)
        {
            ...
            _ExportableFields.ItemsChanged += delegate
            {
                ...
                // 本專案匯出欄位完全由 Exporter.ExportableFields 決定，
                // 不預先插入目前學生基本資料欄位，避免「班級」被目前班級覆寫。
        ...
        private void wizard1_FinishButtonClick(object sender, CancelEventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                bkwNotPassComputer.RunWorkerAsync(new object[] { saveFileDialog1.FileName, exportFieldList, Filler });
                this.Close();
            }
        }
        ...
        void bkwNotPassComputer_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            List<string> exportFieldList = (List<string>)((object[])e.Argument)[1];
            Dictionary<ManualResetEvent, List<RowData>> Filler = (Dictionary<ManualResetEvent, List<RowData>>)((object[])e.Argument)[2];
            double totleProgress = 0.0;
            double packageProgress = Filler.Count == 0 ? 100.0 : 100.0 / Filler.Count;
            ...
            {
                report.Worksheets[0].Cells[0, j].PutValue(exportFieldList[j]);
            }
            RowIndex = 1;
                ...
                if (RowIndex <= 65535)
                {
                    ...
                    foreach (RowData row in Filler[eve])
                    {
                        if (RowIndex <= 65535)
                        {
                            for (int j = 0; j < exportFieldList.Count; j++)
                            {
                                string value = row.ContainsKey(exportFieldList[j])
                                    ? row[exportFieldList[j]]
                                    : "";
                                // 以字串寫入，避免學號前導 0 被當成數字遺失
                                report.Worksheets[0].Cells[RowIndex, j].PutValue(value ?? "");
                            }
                            RowIndex++;
                        }
            ...
            for (int k = 0; k < exportFieldList.Count; k++)
            {
                report.Worksheets[0].AutoFitColumn(k, 0, 150);
            }
            report.Worksheets[0].FreezePanes(1, 0, 1, exportFieldList.Count);
